...
### Check cell quality
figure_name <- ""
figure_name <- paste(mysample, "qc1_vlnplot.pdf", sep="") 
pdf(file=figure_name)
VlnPlot(object = myObject, features = c("nCount_RNA", "nFeature_RNA", "nCount_ATAC", "TSS.enrichment", "nucleosome_signal", "percent.mt"),ncol = 6)
dev.off()
...
figure_name <- ""
figure_name <- paste(mysample, "densityscatter.pdf", sep="") 
pdf(file=figure_name)
DensityScatter(myObject, x = 'nCount_peaks', y = 'TSS.enrichment', log_x = TRUE, quantiles = TRUE)
dev.off()

figure_name <- ""
figure_name <- paste(mysample, "tssplot.pdf", sep="")
pdf(file=figure_name)
...
figure_name <- ""
figure_name <- paste(mysample, "fragmentHist.pdf", sep="")
pdf(file=figure_name)
...
figure_name <- "" 
figure_name <- paste(mysample, "vlpot.pdf", sep="")
pdf(file="vlplot.pdf")
VlnPlot(
  object = myObject,
